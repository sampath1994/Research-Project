...
from sort import *
from pedestrian.counter import get_roi_contour, is_in_roi, get_count_change, is_empty_roi, get_count_in_roi, get_predicted_position
...
def ped(frame, ped_cascade, current_objs, cont, frame_count, ped_count_in_roi, wait_frm_count, mot_tracker, thresh):
    ...
    for i in track_bbs_ids:
        ...
        if is_in_roi(cen_x, cen_y, cont[0]):
            ...
            current_objs.append((obj_id, 'o', cen_x, cen_y))
    frame_count_return = frame_count + 1
    cv2.drawContours(frame, cont, 0, (255, 0, 0), 2, 8)
    chng = get_count_change(pre_objs, current_objs)
    predicted_pos = get_predicted_position(pre_objs, current_objs)
    for pr in predicted_pos:
        c_x, c_y = pr
        cv2.putText(frame, "P", (int(c_x), int(c_y)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
