...
def metadata():
    ...
    b = float(df[1].values)
    print(w, b)
    # find minimum row that gives positive pixel car length
    minimum_row = int(((-1*b) / w)+1)
    Row = np.array(list(range(minimum_row, 700)))
    Ratio = CAR_LENGTH / (w * Row + b)

    print(measure_real_length(300, 320, w, b, CAR_LENGTH))  # just example
